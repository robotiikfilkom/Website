  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-center">
      {stats.map((stat, index) => (
        <div 
          key={stat.label}
          className={`bg-[var(--white)] text-[var(--main-blue)] p-4 rounded-xl ${index === 2 ? 'col-span-2 sm:col-span-1' : ''}`}
        >
          <div className="text-3xl sm:text-4xl md:text-5xl font-bold">
            <RandomNumber n={stat.value} />{stat.suffix}
          </div>
            ...
            <form ref={form} onSubmit={sendEmail} className="space-y-6">
              <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                <div className="w-full">
                  <label htmlFor="from_name" className="block text-sm font-semibold mb-2">Nama</label>
                  <input id="from_name" name="from_name" type="text" required className="w-full px-4 py-3 rounded-md bg-white/10" placeholder="Nama Anda" />
                </div>
                <div className="w-full">
                  <label htmlFor="from_email" className="block text-sm font-semibold mb-2">Email</label>
                  <input id="from_email" name="from_email" type="email" required className="w-full px-4 py-3 rounded-md bg-white/10" placeholder="your.email@company.com" />
                </div>
              </div>
              <div>
                <label htmlFor="subject" className="block text-sm font-semibold mb-2">Subjek</label>
                <input id="subject" name="subject" type="text" required className="w-full px-4 py-3 rounded-md bg-white/10" placeholder="Tujuan Partnership" />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-semibold mb-2">Deskripsi</label>
                <textarea id="message" name="message" required className="w-full px-4 py-3 rounded-md bg-white/10" rows="4" placeholder="Jelaskan tujuan Anda di sini..."></textarea>
              </div>
              <div className="flex justify-end items-center pt-4 gap-4">
                 {formState.message && (
                    <p className={`text-sm ${formState.status === 'success' ? 'text-green-400' : 'text-red-400'}`}>
                      {formState.message}
                    </p>
                 )}
                 <button type="submit" disabled={formState.status === 'submitting'} className="bg-[var(--white)] text-[var(--main-blue)] font-bold px-8 py-3 rounded-full hover:bg-gray-200 transition text-lg flex items-center gap-2 disabled:bg-gray-300">
